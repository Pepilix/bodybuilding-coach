...
,
  ...
  async saveMacroCheckin(p){ const user=await this.user();if(!user)throw new Error('Sign in first.');const row={user_id:user.id,type:'macro',recorded_at:p.recorded_at||new Date().toISOString(),extracted_data:{calories:Number(p.calories),protein_g:Number(p.protein_g),carbs_g:Number(p.carbs_g),fat_g:Number(p.fat_g)},notes:p.notes||'Daily macros'};const {data,error}=await window.bbSupabase.from('checkins').insert(row).select().single();if(error)throw error;return data; },
  async latestMacroCheckin(){ const user=await this.user();if(!user)return null;const {data,error}=await window.bbSupabase.from('checkins').select('*').eq('user_id',user.id).eq('type','macro').order('recorded_at',{ascending:false}).limit(1).maybeSingle();if(error)throw error;return data; },
