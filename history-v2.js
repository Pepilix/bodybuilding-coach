...
(async()=>{const box=document.getElementById('bodyHistory');if(!box)return;try{const rows=await BBDB.latestMetrics();if(!rows.length){box.innerHTML='<p class="muted">No body-composition records yet.</p>';return}box.innerHTML=rows.map(x=>{const vals=[];if(x.weight_kg!=null)vals.push('Weight '+x.weight_kg+' kg');if(x.body_fat_pct!=null)vals.push('Body fat '+x.body_fat_pct+'%');if(x.skeletal_muscle_kg!=null)vals.push('Muscle '+x.skeletal_muscle_kg+' kg');if(x.waist_cm!=null)vals.push('Waist '+x.waist_cm+' cm');if(x.chest_cm!=null)vals.push('Chest '+x.chest_cm+' cm');return '<div class="historySession"><b>'+new Date(x.recorded_at).toLocaleDateString('en-GB')+'</b><p class="muted">'+(x.source||'manual')+'</p><div class="historySets">'+vals.join(' · ')+'</div></div>'}).join('')}catch(e){box.innerHTML='<p class="muted">Body data unavailable: '+e.message+'</p>'}})();