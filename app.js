...
async function renderCycle(){
  ...
  document.querySelectorAll('[data-cycle]').forEach(row=>row.onclick=()=>{const i=+row.dataset.cycle,s=cycle[i],d=document.getElementById('cycle-'+i),open=!d.classList.contains('hidden');document.querySelectorAll('.cycleExpand').forEach(x=>x.classList.add('hidden'));if(open)return;if(s.rest){d.innerHTML='<p><b>Recovery day</b></p><p class="muted">No lifting prescribed. Steps and normal recovery activity only.</p>'}else if(s.exercises?.length){d.innerHTML='<p class="muted">'+(s.emphasis||'Working sets')+'</p>'+s.exercises.map(x=>'<div class="cycleExercise"><b>'+x.exercise.name+'</b><span>'+x.prescribed_sets+' × '+x.rep_min+'–'+x.rep_max+(x.load_unit?' · '+x.load_unit:'')+'</span></div>').join('')+'<a class="cycleStart" href="workout.html?v=187&session='+encodeURIComponent(s.id)+'">Open workout →</a>'}else d.innerHTML='<p class="muted">Session details are not available in the active block.</p>';d.classList.remove('hidden')});
}
renderCycle();
