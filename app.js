...
['photoUpload'].forEach(id=>{let el=document.getElementById(id);if(el)el.onchange=()=>{if(el.files[0]){let log=DB.get('uploads',[]);log.unshift({type:id,name:el.files[0].name,date:new Date().toISOString()});DB.set('uploads',log);document.getElementById('uploads').textContent='Recorded: '+el.files[0].name+' · '+new Date().toLocaleString();}}});
...
const macroBtn=document.getElementById('macroCheckinBtn'),macroForm=document.getElementById('macroForm'),saveMacros=document.getElementById('saveMacrosBtn');
if(macroBtn)macroBtn.onclick=async()=>{macroForm.classList.toggle('hidden');try{const last=await BBDB.latestMacroCheckin();const d=last?.extracted_data||{};if(d.calories!=null)document.getElementById('macroCalories').value=d.calories;if(d.protein_g!=null)document.getElementById('macroProtein').value=d.protein_g;if(d.carbs_g!=null)document.getElementById('macroCarbs').value=d.carbs_g;if(d.fat_g!=null)document.getElementById('macroFat').value=d.fat_g}catch(e){}};
if(saveMacros)saveMacros.onclick=async()=>{const msg=document.getElementById('macroMsg'),p={calories:document.getElementById('macroCalories').value,protein_g:document.getElementById('macroProtein').value,carbs_g:document.getElementById('macroCarbs').value,fat_g:document.getElementById('macroFat').value};if(Object.values(p).some(v=>v==='')){msg.textContent='Enter calories, protein, carbs and fat.';return}saveMacros.disabled=true;saveMacros.textContent='Saving…';try{const saved=await BBDB.saveMacroCheckin(p);saveMacros.textContent='Coach reviewing…';const review=await BBDB.coachReview('nutrition',{date:new Date().toISOString(),...p},saved.id);msg.innerHTML='Saved ✓<br><b>Coach:</b> '+String(review.summary).replace(/</g,'&lt;')+'<br><b>Next:</b> '+String(review.decision||'No change').replace(/</g,'&lt;');saveMacros.textContent='Saved & reviewed ✓';setTimeout(()=>{saveMacros.disabled=false;saveMacros.textContent='Save & Review'},1500)}catch(e){msg.textContent='Not saved: '+e.message;saveMacros.disabled=false;saveMacros.textContent='Save & Review'}};
